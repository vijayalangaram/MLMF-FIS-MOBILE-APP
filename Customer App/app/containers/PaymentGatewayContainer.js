...
import { debugLog, encodeData, getProportionalFontSize, isRTLCheck, RESPONSE_SUCCESS, RESTAURANT_ERROR, RETURN_URL } from "../utils/EDConstants";
import { EDFonts } from "../utils/EDFontConstants";
import { netStatus } from "../utils/NetworkStatusConnection";
import { addOrder, generateAccessTokenAPI, paypalCaptureAPI, paypalOrderAPI, updatePendingOrdersAPI, createOnlinePaymentMethod, driverTipAPI } from "../utils/ServiceManager";
        ...
        netStatus(status => {
            if (status) {
                let param = {
                    ...
                    total_rate: this.isForTip ? parseFloat(this.pendingTotalPayment).toFixed(2) :
                    (parseFloat(this.isForPending ? this.pendingTotal :
                        (this.isPendingAdded && this.promoObj.coupon_id == undefined ? parseFloat(this.pendingTotalPayment) : this.checkoutDetail.total)).toFixed(2).toString()),

...
export class PaymentContainer extends React.PureComponent {
  ...
  state = {
    ...
    payPending:
      this.props.checkoutDetail.order_delivery == "DineIn" ? false : true,
    totalPrice: 0,
  ...
  onSuccessOrderListing = (onSuccess) => {
    ...
    if (onSuccess != undefined && onSuccess.status == RESPONSE_SUCCESS) {
      if (onSuccess.dineIn.length > 0) {
        ...
        this.oldSubtotal = pending[0].price.filter(
          (data) => data.label_key === "Sub Total"
        )[0].value;
        this.oldTotal = pending[0].price.filter(
          (data) => data.label_key === "Total"
        )[0].value;
        ...
        this.setState({
          pendingOrderArray: pending,
          totalPendingAmount: totalPending,
          totalPrice: this.state.totalPrice + parseFloat(totalPending),
          payPending: true,
          later: false,
        });
      } else {
  ...
  onWillFocusPayment = () => {
    if (
      this.checkoutDetail.order_delivery == "DineIn" &&
      this.allowPayLater !== undefined &&
      this.allowPayLater === "1"
    ) {
  ...
  onSuccessPaymentList = (onSuccess) => {
    ...
    if (
      ...
      this.setState({
        ...
        cashOn: onSuccess.Payment_method[0].payment_gateway_slug == "cod",
        later: onSuccess.Payment_method[0].payment_gateway_slug == "paylater",
        online: false,
        disableToggle: false,
        payPending: false,
        totalPrice:
          this.state.totalPrice - parseFloat(this.state.totalPendingAmount),
      });
    ...
    if (!this.state.disableToggle) {
      this.setState({
        ...
        totalPrice:
          ...
          parseFloat(this.state.totalPendingAmount),
        later: false,
        online:
          this.paymentOptions !== null &&
          this.paymentOptions.length !== 0 &&
  ...
  navigateToPendingOrders = () => {
    this.props.navigation.navigate("PendingOrdersFromCart", {
      orderDetails: this.state.pendingOrderArray,
    });
  };
    ...
    if (data.payment_gateway_slug == "cod") {
      {
        ...
        this.setState({
          ...
          online: false,
          later: false,
          selectedOption: data.payment_gateway_slug,
        });
  ...
  onCheckCardPaymentFailure = (onFailure) => {
    ...
    showValidationAlert(
      strings("paymentFail") +
        ...
        onFailure.stripe_response.error !== undefined
          ? onFailure.stripe_response.error.message
          : "")
    );
    this.setState({ isLoading: false });
  };
  ...
  onSuccessAddCart = (onSuccess) => {
    ...
    if (onSuccess.error != undefined) {
      ...
      if (onSuccess.status == RESPONSE_SUCCESS) {
        ...
        this.checkoutDetail.is_creditcard_fee_applied =
          onSuccess.is_creditcard_fee_applied;
        this.checkoutDetail.creditcard_feeval = onSuccess.creditcard_feeval;
        this.checkoutDetail.creditcard_fee_typeval =
          onSuccess.creditcard_fee_typeval;
        ...
        if (
          this.state.totalPendingAmount !== 0 &&
          this.state.selectedOption == "paylater"
        )
          this.setState({
            payPending: false,
            later: true,
        ...
        if (
          this.addToCartData.is_creditcard == "yes" &&
          this.oldSubtotal !== undefined &&
          this.oldSubtotal !== null &&
          this.oldSubtotal !== ""
        ) {
          if (onSuccess.creditcard_fee_typeval == "Amount") {
          } else if (onSuccess.creditcard_fee_typeval == "Percentage") {
            this.old_credit_fee =
              Number(this.oldSubtotal) *
              (Number(onSuccess.creditcard_feeval) / 100);
          }
        }

        this.setState({
          totalPrice:
            parseFloat(onSuccess.total) +
            (this.state.selectedOption == "paylater"
              ? 0
              : parseFloat(this.state.totalPendingAmount)),
        });
      ...
      >
        ...
        >
          ...
          <View style={style.mainView}>
            <View>
              {this.state.pendingOrderArray !== undefined &&
              this.state.pendingOrderArray.length !== 0 ? (
                <View style={style.pendingContainer}>
                  <View
                    style={[
                      style.pendingSubContainer,
                      { flexDirection: isRTLCheck() ? "row-reverse" : "row" },
                    ]}
                  >
                    <Text style={style.simpleText} onPress={this.togglePending}>
                      {this.state.payPending
                        ? strings("payPending")
                        : strings("pendingOrderMsg")}
                    </Text>
                  </View>
                  <View
                    style={[
                  ...
                  <View style={style.cartResponse}>
                    ...
                    >
                      <Text style={[style.cartResponseTextStyle, { flex: 1 }]}>
                        {this.promoObj.coupon_arrayapply.length +
          ...
          label={
            isRTLCheck()
              ? strings("continue") +
                "- " +
                funGetFrench_Curr(
                  (this.state.discountedPrice !== undefined
                    ? this.state.discountedPrice
                    : this.state.totalPrice) + this.old_credit_fee,
                  1,
                  this.props.currency
                ) +
                this.props.currency
  ...
  applyPromo = () => {
    netStatus((status) => {
      if (status) {
        ...
        applyCouponAPI(
          promoParams,
          this.onSuccessApplyCoupon,
          this.onFailureApplyCoupon,
          this.props
        );
  ...
  onSuccessApplyCoupon = (onSuccess) => {
    ...
    if (
      ...
      this.setState({
        discountedPrice:
          Number(this.oldSubtotal) +
          this.updatedCartResponse.total -
          onSuccess.coupon_discount,
      });
      if (onSuccess.status == COUPON_ERROR)
        showValidationAlert(onSuccess.message);
    } else showValidationAlert(onSuccess.message);
    ...
    netStatus((status) => {
      if (status) {
        let param = {
          ...
          payment_option:
            this.tempPayment_option !== undefined &&
            this.tempPayment_option !== null &&
  ...
  onSuccessUpdateOrder = (onSuccess) => {
    if (onSuccess.status == RESPONSE_SUCCESS) {
      ...
      this.props.navigation.navigate("OrderConfirm", {
        isForDineIn: this.state.order_delivery == "DineIn" ? true : false,
        resObj: this.resObj,
        ...
        clearCartData(
          (response) => {
            if (this.state.payPending) {
              ...
              this.props.navigation.navigate("OrderConfirm", {
                isForDineIn:
                  this.state.order_delivery == "DineIn" ? true : false,
                resObj: onSuccess.restaurant_detail,
                navigateToOrder: !this.state.later,
                cashback: onSuccess.earned_wallet_money,
              });
            }

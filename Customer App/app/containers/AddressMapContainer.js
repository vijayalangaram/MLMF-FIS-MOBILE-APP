...
class AddressMapContainer extends React.Component {
  ...
  onFailureDeleteAddress = (onFailure) => {
    this.setState({ isLoading: false });
    showValidationAlert(
      onFailure.addAddress || strings("generalWebServiceError")
    );
  };
  ...
  render() {
    return (
      <BaseContainer
        title={
          this.props.navigation.state.params.isEdit == 2
            ? strings("editAddress")
            : strings("addAddress")
        }
        left={"arrow-back"}
      ...
      >
        <View
          pointerEvents={
            this.state.isLoading || this.state.isCurrentLoading
              ? "none"
              : "auto"
          }
        ...
        >
          ...
          >
            <View style={style.subContainer}>
              ...
              <MapView
                pointerEvents={
                  this.state.isCurrentLoading || this.state.isLoading
                    ? "none"
                    : "auto"
                }
            ...
            <View style={style.checkboxView}>
              <TextInput
                ...
                onChangeText={(text) =>
                  this.textFieldTextDidChangeHandler(text, "address_label")
                }
              />
              <EDRTLText
              ...
              <TextInput
                ...
                onChangeText={(text) =>
                  this.textFieldTextDidChangeHandler(text, "strAddress1")
                }
              />
          ...
          <View
            style={[
              style.btnView,
              {
                bottom:
                  Platform.OS == "ios"
                    ? 15 + initialWindowMetrics.insets.bottom
                    : 15,
              },
            ]}
          >
  ...
  componentDidMount() {
    ...
    if (this.state.totalCount == 0) this.setState({ isHome: true });
    this.viewUpdate();
  }
  ...
  viewUpdate = () => {
    if (
      ...
      !(this.allData.addressId === "") &&
      this.allData.addressId != undefined
    ) {
      ...
      this.state.objRegistrationDetails.address_label =
      ...
      setTimeout(
        () => this.locationRef.setAddressText(addressData.addressLine2),
        100
      );
    } else {
      if (
    ...
    if (!this.loading && !this.regionloading) {
      this.setState({
        region: {
          ...
          latitudeDelta: this.regionComplete
            ? region.latitudeDelta
            : LATITUDE_DELTA,
          longitudeDelta: this.regionComplete
  ...
  saveAddressFromSearch = (data, details) => {
    ...
    let address = getAddressFromAddressComponent(
  ...
  onAddressSave = () => {
    ...
    if (
      ...
      this.state.strAddressNew.trim() !== ""
    )
      this.callAddAddressAPI();
    else showValidationAlert(strings("properAddressMessage"));
  ...
  callAddAddressAPI = () => {
    if (this.isGuest) {
      this.props.navigation.state.params.getGuestAddress({
        ...
        address:
          this.state.strAddress2.trim().length !== 0
            ? this.state.strAddress2
            : this.state.strAddressNew.trim().length !== 0
            ? this.state.strAddressNew
            : "",
        latitude: this.state.latitude,
        longitude: this.state.longitude,
        city: this.city,
...
export const style = StyleSheet.create({
  ...
  searchInputContainer: {
    backgroundColor: EDColors.white,
    borderTopColor: EDColors.transparent,
    flex: 1,

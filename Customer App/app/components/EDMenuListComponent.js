...
export default class EDMenuListComponent extends React.PureComponent {
  constructor(props) {
    ...
    this.state = {
      ...
      key: 1,
      headerHeight: metrics.screenHeight * 0.26,
      tabHeight: 45,
    ...
    this.getItemLayout = sectionListGetItemLayout({
      ...
      listHeaderHeight:
        metrics.screenHeight * 0.585 + (this.props.isOpen ? 0 : 40), // The height of your list header
    });
  }
  ...
  componentWillMount = () => {
    ...
    if (
      (this.props.popularItems != undefined) &
        (this.props.popularItems != null) &&
      this.props.popularItems.length > 0
    ) {
      // this.sectionData.push({ category_id: -1, title: strings("popularItems"), data: this.props.popularItems })
      this.setState({ selectedID: -1 });
    } else this.setState({ selectedID: undefined });
  ...
  componentWillReceiveProps = (newProps) => {
    if (newProps.isSearchVisible == true) {
      if (this.sectionRef !== undefined && this.sectionRef !== null) {
        ...
        setTimeout(() => {
          if (this.textInput) this.textInput.textInput.focus();
        }, 100);
      }
    }
  };
  ...
  menuItemsRender = (item, section) => {
    return section.category_id == -2 ? (
      ...
      <ProductComponent
        ...
        cartData={
          this.props?.cartData != undefined && this.props?.cartData.length === 0
            ? []
            : this.props.cartData
        }
  ...
  onScrollViewScrolll = (event) => {
    if (event.nativeEvent.contentOffset.y >= this.state.headerHeight) {
      ...
      this.state.headerHeight - this.state.tabHeight
    ) {
  ...
  onCheckViewableItem = ({ viewableItems, changed }) => {
    if (
      ...
      let index = this.sectionData
        .map((data) => data.category_id)
        .indexOf(viewableItems.reverse()[0].section.category_id);
      if (index !== -1) {
        try {
          ...
          (err) => {
            debugLog("TAB SCROLL ERROR :::::", err);
          };
        }
      }
      let imagesToLoad = [
        ...this.state.imagesToLoad,
        ...viewableItems.map((data) => data.section.category_id),
      ];

      this.setState({
        selectedID: viewableItems.reverse()[0].section.category_id,
  ...
  onHeaderLayout = (e) => {
    this.setState({
      headerHeight: e.nativeEvent.layout.height - this.state.tabHeight,
      imagesToLoad:
  ...
  ListHeaderComponent = () => {
    return (
      <View style={{}} onLayout={this.onHeaderLayout}>
  ...
  onTabPress = (index) => {
    ...
    this.sectionRef.scrollToLocation({
      ...
      itemIndex: 0,
      animated: true,
      viewOffset: this.props.isSearchVisible
        ? this.state.searchHeight + 38
        : 38,
      viewPosition: 0,
    });
  };
        ...
        style={[
          styles.footerImage,
          {
            marginTop:
              ...
              this.sectionData[this.sectionData.length - 1].data.length *
                (metrics.screenHeight * 0.175 + 20),
        ...
        renderSectionHeader={({ section }) => {
          return section.category_id == -2 ? (
            ...
            <View style={{ height: 60 }}>
              ...
              >
                {section.title != undefined ? section.title.toUpperCase() : ""}
              </Text>
            </View>
          );
        }}
  ...
  renderTab = (key = 1) => {
    return (
      <FlatList
        ...
        renderItem={(item) => {
          return item.item.category_id == -2 ? (
            ...
            <EDButton
              ...
              style={{
                ...
                borderRadius: 10,
                backgroundColor:
                  this.state.selectedID == item.item.category_id
                    ? EDColors.primary
                    : EDColors.white,
              }}
              textStyle={{
  ...
  onPullToRefreshHandler = () => {
    // this.refreshing = false
    // this.forceUpdate();
    if (this.props.onPullToRefreshHandler) this.props.onPullToRefreshHandler();
  };
  ...
  onSearch = async (text) => {
    ...
    menu.map((item, index) => {
      menu[index].data = menu[index].data.filter((data) => {
        return (
          data.name.toLowerCase().includes(text.trim().toLowerCase()) ||
    ...
    if (menu.length !== 0) this.sectionData = menu;
    else
      this.sectionData = [
        {
          category_id: -2,
      ...
      setTimeout(
        () =>
          this.sectionRef.scrollToLocation({
            ...
            itemIndex: 0,
            animated: true,
            viewOffset: this.state.searchHeight + 5,
            viewPosition: 0,
          }),
        50
      );
    ...
    const imageOpacity = this.scrollY.current.interpolate({
      inputRange: [
        ...
        this.state.headerHeight -
          (this.props.isSearchVisible ? this.state.searchHeight : 0),
        this.state.headerHeight -
          (this.props.isSearchVisible ? this.state.searchHeight : 0) +
          (Platform.OS == "ios" ? 5 : 0),
      ],

...
import { capiString, debugLog, getProportionalFontSize, GOOGLE_WEBCLIENT_ID, isRTLCheck } from "../utils/EDConstants";
...
class SideBar extends React.PureComponent {
    ...
    setupSideMenuData = () => {
        ...
        return [
            ...
            { route: 'PendingOrders', screenName: strings("activeDineInOrder"), icon: "fast-food-outline", type: 'ionicon' },
    ...
    render() {
        ...
        let arrCMSPages = ((cmsData)).map(itemToIterate => { return { isAsset: true, route: 'CMSContainer', screenName: itemToIterate.name, icon: { uri: itemToIterate.cms_icon }, cmsSlug: itemToIterate.CMSSlug }; });
        let arraySideMenuData = arrTemp.concat(arrCMSPages);
        arraySideMenuData = arraySideMenuData.concat(
            [{ route: 'contactUs', screenName: strings('contact'), icon: 'chatbubble-ellipses-outline', type: "ionicon" },
        ...
        return (
            <View style={{ flex: 1, backgroundColor: "#F5F5F5" }} pointerEvents={this.state.isLoading ? 'none' : 'auto'} >
                ...
                <TouchableOpacity
                    style={{ flex: 1, paddingTop: Platform.OS == "ios" ? initialWindowMetrics.insets.top + 5 : 5 }}
                    activeOpacity={1.0}
                ...
                <View style={style.navItemContainer}>
                    ...
                    {/* VERSION DETAIL */}
                    <EDRTLView style={[style.versionStyle, { alignItems: isRTLCheck() ? 'flex-end' : 'flex-start' }]}>
                        <EDRTLView style={style.logoView}>
                            ...
                            <Text style={style.iconText} >{'V.1.1 14.7.23'}</Text>
                        </EDRTLView>
    ...
    onDrawerItemPressed = (selectedIndex) => {
        ...
        if (this.arrayFinalSideMenu[selectedIndex].screenName === strings('signout')) {
            ...
            this.props.navigation.closeDrawer();
            this.props.saveNavigationSelection(this.arrayFinalSideMenu[selectedIndex].route == "CMSContainer" ? this.arrayFinalSideMenu[selectedIndex].screenName : this.arrayFinalSideMenu[selectedIndex].route);
